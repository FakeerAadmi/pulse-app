/* =========================================================
        ...
        <line x1="9" y1="10" x2="9" y2="10.01" stroke-width="2"/>
        <line x1="15" y1="10" x2="15" y2="10.01" stroke-width="2"/>
    </svg>`,
            `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
        <circle cx="12" cy="12" r="9"/>
        ...
        <line x1="15" y1="10" x2="15" y2="10.01" stroke-width="2"/>
            ...
            `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
        <circle cx="12" cy="12" r="9"/>
        <path d="M8.5 15.5s1-1.5 3.5-1.5 3.5 1.5 3.5 1.5"/>
        <path d="M8.5 9.5c.5-.5 1.5-.5 2 0M13.5 9.5c.5-.5 1.5-.5 2 0"/>
    </svg>`,
            `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
        <circle cx="12" cy="12" r="9"/>
        <path d="M8.5 15.5s1-1.5 3.5-1.5 3.5 1.5 3.5 1.5"/>
        <line x1="8.5" y1="9.5" x2="10.5" y2="10.5" stroke-width="1.5"/>
        <line x1="15.5" y1="9.5" x2="13.5" y2="10.5" stroke-width="1.5"/>
    </svg>`,
            `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
        <circle cx="12" cy="12" r="9"/>
        <line x1="8.5" y1="14.5" x2="15.5" y2="14.5"/>
        <path d="M9 10.5c0-.5.5-1 1-1M14 10.5c0-.5.5-1 1-1"/>
        <line x1="12" y1="6" x2="12" y2="8" stroke-width="1.5"/>
    </svg>`,
        ];

        const P = {
            cigarette: {
                label: 'Cigarette', icon: ICONS.cigarette, short: 'Cig',
                strengths: [{ l: 'Light', mg: .6 }, { l: 'Regular', mg: 1.2 }, { l: 'Strong', mg: 1.8 }], def: 1, combustible: true
            },
            cigar: {
                label: 'Cigar', icon: ICONS.cigar, short: 'Cigar',
                strengths: [{ l: 'Mild', mg: 3 }, { l: 'Medium', mg: 7 }, { l: 'Full', mg: 15 }], def: 0, combustible: true
            },
            cigarillo: {
                label: 'Cigarillo', icon: ICONS.cigarillo, short: 'Cigaro',
                strengths: [{ l: 'Mild', mg: 1 }, { l: 'Regular', mg: 2 }, { l: 'Strong', mg: 4 }], def: 1, combustible: true
            },
            pipe: {
                label: 'Pipe', icon: ICONS.pipe, short: 'Pipe',
                strengths: [{ l: 'Light', mg: 1 }, { l: 'Regular', mg: 3 }, { l: 'Rich', mg: 6 }], def: 1, combustible: true
            },
            vape_pod: {
                label: 'Vape (Pod)', icon: ICONS.vape_pod, short: 'Pod',
                strengths: [{ l: '3mg', mg: .5 }, { l: '6mg', mg: .8 }, { l: '10mg', mg: 1 }, { l: '20mg Salt', mg: 1.5 }], def: 1
            },
            vape_mod: {
                label: 'Vape (Mod)', icon: ICONS.vape_mod, short: 'Mod',
                strengths: [{ l: '3mg', mg: .4 }, { l: '6mg', mg: .7 }, { l: '12mg', mg: 1 }, { l: '18mg', mg: 1.4 }], def: 1
            },
            patch: {
                label: 'Patch', icon: ICONS.patch, short: 'Patch',
                strengths: [{ l: '7mg/d', mg: .3 }, { l: '14mg/d', mg: .6 }, { l: '21mg/d', mg: 1 }], def: 2
            },
            gum: {
                label: 'Gum', icon: ICONS.gum, short: 'Gum',
                strengths: [{ l: '2mg', mg: 1.5 }, { l: '4mg', mg: 3 }], def: 0
            },
            pouch: {
                label: 'Pouch', icon: ICONS.pouch, short: 'Pouch',
                strengths: [{ l: '4mg', mg: 3 }, { l: '8mg', mg: 6 }, { l: '12mg', mg: 9 }, { l: '16mg', mg: 12 }], def: 0
            },
        };
        ...
        const state = {
            ...
            tourSeen: false,
            plannedGap: null,          // timestamp of user's next planned intake
            plannedGapMins: null,      // which pill was selected (60/120/180/360)
            mode: 'Understand patterns',
            reflection: {
                lastPromptDate: null,  // date string of last prompt
                answers: [],           // [{date, trigger, note}]
            },
            weeklySummary: {
                lastGenDate: null,
                text: '',
        ...
        function updateMetabolism() {
            ...
            let baseHL = 120; // baseline half-life in minutes

            // Sex: female CYP2A6 activity ~30% faster
            if (p.sex === 'F') baseHL *= 0.72;
            else if (p.sex === 'O') baseHL *= 0.88;

            // Age: clearance slows after 60, slightly faster under 25
            if (p.age > 60) baseHL *= 1.20;
            else if (p.age < 25) baseHL *= 0.90;

            // Weight: affects volume of distribution
            ...
            // Tolerance ceiling rises with years of use, plateaus at 20 yrs
        ...
        /* ─────────────────────────────────────────────────────────
           SMART GRAPH WINDOW
           Always shows: [first relevant intake - pad] → [baseline + pad]
           "now" marker sits wherever it naturally falls in that span.
           ───────────────────────────────────────────────────────── */
        function getGraphWindow(rangeMs) {
            const now = Date.now();
            // Grab intakes within the lookback window (+ 30 min buffer before start)
            const relevant = state.intakes.filter(i => i.time >= now - rangeMs - 30 * 60000);

            if (relevant.length === 0) {
                // No data: centre now with 72/28 split
            ...
            // 12 min breathing room before first intake, 18 min after baseline
            ...
            // Enforce minimum visible span so the graph isn't too narrow
            const span = winEnd - winStart;
            const minSpan = 40 * 60000; // 40 minutes minimum
            if (span < minSpan) {
        ...
        function finishOnboarding() {
            ...
            // Start drawing the app loop
            tick();

            // Launch tour after a brief pause for the UI to settle
        ...
        function toggleAmoled() {
            ...
            if (state.amoledMode && !state.darkMode) {
                toggleDark(); // Force dark mode on if AMOLED is selected
            }
        }
        ...
        function drawGraph() {
            ...
            // ── Window layout via smart auto-fit ──────────────────────────
            // Always shows from first recent intake to baseline crossing,
            // with a "now" marker wherever the current moment falls.
            ...
            // "Now" divider — subtle vertical line
            ...
            if (!hasLogs) {
                // Animated baseline sine across full window
                ...
                // ── Dynamic y-scale based on peak in window ──
                let maxSat = .05;
                ...
                // ── Build points for the PAST portion (winStart → now) ──
                const pastPts = [];
                ...
                // ── Build points for the FUTURE projection (now → winEnd) ──
                const futurePts = [];
                ...
                // ── Time axis labels — clock times at even intervals ──
                ctx.fillStyle = `rgba(${inkBase},.36)`;
                ctx.font = `7px 'JetBrains Mono',monospace`; ctx.textAlign = 'center';
                // Draw 4 evenly-spaced time labels
                for (let i = 0; i <= 4; i++) {
                ...
                // ── Dotted fill under FUTURE projection ──
                ...
                if (currentSat > 0.05) {
                    ...
                    if (baselineX > nowX && baselineX < rW + 40) {
                        // Draw a small tick at baseline crossing
                ...
                // ── Event markers (intake dots) ──
                state.intakes.forEach(log => {
                    const lx = tx(log.time);
                    if (lx < 4 || lx > nowX + 2) return; // only past events
        ...
        function renderExpandedGraph() {
            ...
            if (isMultiDay) {
                ...
                // ── LINE CHART — smart window, scrollable canvas ──
                const { winStart, winEnd } = getGraphWindow(rangeMs);
                const winSpan = winEnd - winStart;

                // 1px per 30s minimum; never narrower than viewport
                ...
                // Auto-scroll: nowX at ~30% from left edge
...
`;
      ...
      <div class="ov-hist-day-bar">
        ...
        function switchScreen(id) {
            ...
            // Reset collapsing titles to full size when entering a screen
            const titleMap = {
      ...
      <button class="brand-chip${state.logSel.brand === b ? ' act' : ''}" onclick="selectBrand('${b.replace(/'/g, "\\'")}')">
        ...
        function submitLog() {
            ...
            // Clear planned gap — user acted on it
        ...
        function renderHistory() {
            ...
            // For history view, add demo entries for past days
  ...
  `;
    ...
    <div class="settings-row" style="flex-direction:column;align-items:flex-start;gap:.45rem">
      ...
      `).join('')}</div>
            ...
            // Brand settings — collapsible categories
      ...
      <div class="brand-cat-body">
        ...
        window.generateShareCard = function() {
            ...
            // Add some subtle noise/texture if we want, or just a simple gradient
            ...
            ctx.fillStyle = '#5a9c8f'; // Sage color
        ...
        function updateMode() {
            ...
            // Each mode adjusts the default daily budget and how Pulse frames its messaging
            const modeDefaults = {
        ...
        function setPlannedGap(mins) {
            ...
            state.plannedGapMins = mins; // remember which pill was tapped
            renderPlannedGap();
        }
        ...
        function renderPlannedGap() {
            ...
            if (state.plannedGap && state.plannedGap > Date.now()) {
                ...
                pills.forEach(p => {
                    const pillMins = parseInt(p.textContent.replace(/\D/g, ''));
                    const match = state.plannedGapMins && Math.abs(pillMins - (state.plannedGapMins > 60 ? state.plannedGapMins / 60 : state.plannedGapMins)) < 0.1;
        ...
        function maybeShowReflection() {
            ...
            // Show after 8pm if user has logged at least 2 intakes today
        ...
        function renderReflectionCard(todayLogs) {
            ...
            // Find the most common trigger today
            const trgCounts = {};
        ...
        function forceWeeklySummary() {
            // Reset gate so it regenerates even if already shown this week
            state.weeklySummary.lastGenDate = null;
            generateWeeklySummary();
            // Switch to home to show the card
            switchScreen('home');
        }

        function maybeShowWeeklySummary() {
            // Show if user has at least 7 days of data and hasn't seen it this week
        ...
        /* ── Collapsing headers for Insights / History / Settings ── */
        ...
        function openTour() {
            // Always navigate to home screen first so tour targets are visible
            switchScreen('home');
            _tourStep = 0;
            setTimeout(() => {
        ...
        function renderTourStep() {
            ...
            // Navigate to the correct screen for this step if needed
            ...
            if (currentScreenId !== targetScreen) {
                ...
                // Re-render after screen transition settles
        ...
        function _doRenderTourStep(step, total) {
            ...
            if (targetEl) {
                ...
                // Position card: prefer below, fall back to above
   ...
   ========================================================= */
        ...
        /* =========================================================
           BIOMETRIC SIMULATION (replaced by native bridge in app)
           window.PulseNative.getHRV() etc. when native
           ========================================================= */
        let _bioSampleTime = 0;
        let _bioStable = null; // stable snapshot, only refreshes every 30s
        ...
        function simulateBiometrics() {
            ...
            // Resample only every 30 seconds so numbers don't flicker
        ...
        function updateBiometrics() {
            ...
            simulateBiometrics(); // swap with PulseNative calls when native
            const b = state.biometrics;
            ...
            if (cp && state.userProfile.sex === 'F') {
                ...
                // Apply cycle multiplier to metabolism
                const cycleHL = (Math.LN2 / K) / cp.mult;
                // display-only adjust — don't permanently mutate K
        ...
        function drawHrvSparkline() {
            ...
            // Deterministic pseudo-random seeded by date (day changes daily, no per-frame jitter)
        ...
        function showBioDeltaToast() {
            ...
            // ── Inline delta badges on each bio card ──
            ...
            setDelta('dRhr', b.rhr, prev.rhr, true);   // lower RHR = good
            setDelta('dSpo2', b.spo2, prev.spo2, false);  // higher SpO₂ = good
            setDelta('dStress', b.stress, prev.stress, true);   // lower stress = good
            setDelta('dHrv', b.hrv, prev.hrv, false);  // higher HRV = good

            // ── Toast for notable multi-metric changes ──
            const lines = [];
        ...
        (function () {
            ...
            function frame() {
                ...
                if (t > 0.05) {
                    ...
                    // In dark mode always use white; in light mode use luminance to decide
                ...
                if (t < 0.01) {
                    // Near-empty: just tiny ripple at bottom
        ...
        /* ═══════════════════════════════════════════════
           RECOVERY RINGS — inserted into Insights screen
           ═══════════════════════════════════════════════ */
        function renderRecoveryRings() {
            ...
            // Last any-product intake (for HRV + circulation rings)
            const lastAny = state.intakes.length > 0
                ? Math.max(...state.intakes.map(i => i.time))
                : null;

            // Last combustible intake (for CO ring)
            const combustibleLogs = state.intakes.filter(l => P[l.product]?.combustible);
        ...
        function startTour() {
            // Close settings, go home, then launch tour overlay directly
            switchScreen('home');
            setTimeout(() => {
                // Reset and launch the feature tour (not onboarding disclaimer)
                _tourStep = 0;
                const ov = document.getElementById('tourOverlay');
                if (ov) {
